...
\begin{document}
   ...
   \title{On the Construction of Stellar  Classification Templates Library for LAMOST Spectra Analysis Pipeline
%\,$^*$
...
%% For single author or all the authors from an institute, use "\inst{}" only
   ...
   \institute{
    Key Laboratory of Optical Astronomy,  Chinese Academy of Sciences, Beijing, 100012, China
      {\it lal@lamost.org}\\
      \and
...
\abstract{
The aim of LAMOST Spectra Analysis Pipeline (also called 1D Pipeline) is to classify and measure the spectra of objects observed in the survey.
Stars are classified into different sub-classes by matching to spectra templates.
To make the classification better and reliable, a new LAMOST stellar spectral classification templates library is constructed.
This paper presents the construction of the templates through the Principal Component Analysis(PCA) of about one million LAMOST stellar spectra.
These spectra were selected from all released 1,946,429 stellar spectra  in LAMOST Data Release One (DR1).
Convolved  with the SDSS \emph{ugriz} filter  curve, each spectrum is assigned a pseudo g-r color (hereafter g$^*$-r$^*$).
All spectra are divided into different groups by the  g$^*$-r$^*$.
Additional groups are formed by different subclasses classified with the current pipeline.
In addition, some special types of psectra are manually picked out to add into the groups.
In each group,  we exclude some outliers and then apply PCA method to  the remaining  spectra.
All spectra are reconstructed using the first few principal components and the weighted average spectra are constructed as the template spectra in the groups.
Each template spectrum  is labeled a MK  class  by comparing with some label-known templates and the spectra of MK standard stars  .
All the templates are visually inspected and  some low-quality spectra are excluded.
Additionally, some unlabeled or wrongly labeled spectra are relabeled  or abandoned.
The finally left spectra compose the template library.
The first version of the library contains * spectra and * different MK classes, which is available to public on the web site.
...
\label{sect:intro}
The LAMOST survey\citep{cui2012large,zhao2012lamost} contains two main parts: the LAMOST ExtraGAlactic Survey (LEGAS) and the LAMOST Experiment for Galactic Understanding and Exploration survey of Milky Way stellar structure(LEGUE, see \citet{deng2012lamost}).
The unique design of LAMOST enables it to take 4000 spectra in a single exposure to a limiting magnitude as faint as r=19 at the resolution R=1800, which is equivalent to the design goal of r=20 for the resolution R=500.
The LAMOST  therefore has great potential to efficiently survey a large volume of space for stars and galaxies.


The LAMOST spectral analysis pipeline\citep{luo2001steps,luo2004design,luo2008automated,wang2010calibration,luo2012data} is based on the specBS pipeline\citep{aihara2011eighth} for the analysis of SDSS spectra.
This analysis code carries out $\chi^2$ fits of the spectra to templates in wavelength space (in the spirit of \cite{glazebrook1998automatic}),
...
which also contain galaxies, QSOs and stars (categorized in terms of subtypes), are used to construct the templates.
With more spectra with high quality observed, it is necessary and better to construct a new template library based on the  spectra  observed by LAMOST.


In this paper, we described  in detail the construction of the LAMOST stellar  classification template library.
The paper is processed as follows:
Section \ref{sect:Sample} describes the used spectra from LAMOST Data Release One (DR1).
The detailed description of the template library construction steps, the related methods and the group dividing are outlined in section \ref{sect:construction}.
The results and discussions are given in \ref{sect:Results}.
A brief summary is given in section \ref{sect:Summary}.

\section{The Spectra From LAMOST Data Release One(DR1)}
\label{sect:Sample}
The first data release of LAMOST survey contains the spectra in the pilot survey and the first year of general survey.
The pilot survey of LAMOST  was launched on 2011 Oct 24, and ended in June 2012.% and about 31,900 spectra were released\citep{luo2012data}.
The first year of LAMOST general survey began from September  28th 2012 and ended on June 3rd 2013.


The  survey has obtained spectra of stars in the Milky Way,
...
The wavelength coverage is from 3700 \AA\  to 9100 \AA.
...
The raw data have been reduced with LAMOST 2D pipeline\citep{bai2012lamost} including bias subtraction, cosmic-ray removal, spectral trace and extraction, flat-fielding, wavelength calibration sky subtraction, and combination.
For the spectra with SNR$>$5, the 1D pipeline\citep{wang2010calibration} gives spectral type and redshift.
...
We exclude those spectra in the Galactic Anti Center and M31 to avoid the high effect of interstellar dust  extinction on the spectra
and 742,669 spectra are left.
Some other spectra are  excluded in the construction of the library, which will be discussed in the section \ref{sect:construction}.
% \cite{york2000sloan}


\section{The construction  steps}
...
The photometric data of objects are from different surveys.
Meanwhile,  the flux calibration is relative not absolute\citep{song2012relative}.
Consequently, we can not get accurate and uniform  colors for LAMOST spectra.
Here, we propose a pseudo g-r color(hereafter g$^*$-r$^*$) obtained by convolving each observed spectra with the SDSS $ugriz$ filter response curves .
...
\begin{enumerate}
 ...
 \begin{equation}
        g^*-r^*=-2.5*log\frac{\sum F_g*C_g}{\sum C_g} +2.5*log\frac{\sum F_r*C_r}{\sum C_r}
       \end{equation}
\end{enumerate}

We select these objects with SDSS $ugriz$ filter magnitude(from different surveys) and $SNR>20$.
The diagram of g-r color and $g^*-r^*$ of these objects is  shown in Fig.\ref{Fig2}.
...
\end{equation}
 ...
 \begin{figure}
   \centering
   \includegraphics[width=12cm, angle=0, clip=true]{f1.eps}
   ...
   Among these spectra, we also choose these spectra classified as O, B, A, F, G, K, M-type.
   For these spectra, the average value and standard deviation of $g^*-r^*$ in each class are also calculated.
 ...
 \begin{figure}
   ...
   The g-r color  is a very good indicator of effective temperature. %\citep{lee2008segue,ivezic2008milky}.
For SDSS spectra, \citet{ivezic2008milky} derived a relation between effective temperature and the color g-r in the $-0.3 < g-r < 1.3$ color range:
\begin{equation}
...
For some spectra classified as A, F, G, K-type, the effective temperatures($T_{eff}$), surface gravities(Log g) and metallicities([Fe/H]) determined by the LASP(LAmost Stellar Parameter pipeline, see \citet{wu2011automatic}) are provided.
...
As shown in Fig.\ref{Fig3}, these two formulas \ref{eq3} and \ref{eq4}  nearly  coincide with each other in the Teff range [5500,7000].
 \begin{figure}
   \centering
...
We select these spectra with $g^*-r^*$ in  the range[-1.5,2.0] and divide all spectra into 175 groups with 0.02 mag width interval.
The number distribution is shown in Figure \ref{Fig4}.
We mark these these groups with group-id from 1 to 175.
 \begin{figure}
   \centering
   \includegraphics[width=8cm, angle=0,clip]{f2.eps}
   \caption{The number distribution of spectra in each $g^*-r^*$ bin.
   The blue line is all spectra while the red line is the spectra with $SNR>10$.
   }
   \label{Fig4}
   \end{figure}
...
The distribution is given is Table \ref{tab1}.
...
We mark these these groups with group-id from 176 to 251.
%
%In addition, we have changed some spectra's subclasses and add some new spectra.
%  \begin{itemize}	

 \citet{yi2013m} present a spectroscopic catalog of 67,082 M dwarfs from LAMOST Pilot Survey.
...
We have divided these spectra into groups with group id as shown in Table \ref{tab1}.
 \citet{zhao201370}  present a spectroscopically identified catalog of 70 DA white dwarfs (WDs).
...
We combine these two catalogs and add the spectra into group 250.
 \citet{jiang2013data} report the identification of 10 cataclysmic variables.
...
180&A1IV&653&&206&Carbon\_lines&10&&231&L5&4\\
...
\begin{enumerate}
					...
					positive integer $k$,  the $k$-distance of object $p$, denoted as
...
We use the LoOP method to exclude these outliers in each group.
%In our work, we use LoOP to exclude outliers in each group to get a better template spectra.
The distance measurement function used in our work is the cosine distance $d=1-\frac{A*B}{|A|*|B|}$
...
\item De-redshift the spectra and unify the wavelength to 3800\AA-9000\AA \ with fixed step 1\AA(the amount of all sampling points is N=5201) and then get the unified flux F for each spectrum.
 ...
 \item These spectra with  $LoOP\ge0.4$ are excluded.
      ...
      \item Do PCA of all selected spectra in each group to  get a feature matrix $T$ and the respective eigen values $\lambda$.
     ...
     \item Reconstruct each selected spectra using obtained first $k$ principal components
...
We compare these spectra with three libraries and label each spectrum a subclass.

\citet{danks1994atlas} presented spectra  for MK standards in the wavelength range    5800\AA-10200\AA.
The stars cover the normal spectral types O to M and    luminosity types I, III, and V.
...
\citet{bolton2012spectral} describe the detail of the pipeline for SDSS III and publish the template used on the web page\footnote{\url{http://www.sdss3.org/svn/repo/idlspec2d/tags/v5_4_45/templates/}}.
For stellar spectral classification,  123 templates are provided which are created from the full database of Indo-U.S. spectra.
...
And those figures are used for following visual inspection.
...
%Finally,  there are * spectra and * different MK classes are left in the template library.


\section{Results and  discussions}
\label{sect:Results}
A updated library is formed after adding some spectra of new subclasses and replacing some spectra.
These spectra of these types not existing in our library are left.
...
The finally constructed template spectra of these groups are as shown in Fig.\ref{Fig6}.
...
\tablecomments{0.86\textwidth}{Subclass is the final MK class.
...
There are totally 17,231 spectra and 325 spectra are selected from first 5,000 spectra with the highest SNR.
...
Compare with \cite{mcgurk2010principal}
\begin{table}
\bc
\begin{minipage}[]{100mm}
\caption[]{The   \label{tab3}}\end{minipage}
\setlength{\tabcolsep}{1pt}
\small
...
why not directly use SDSS DR9 template


\section{Summary}
\label{sect:Summary}
To improve the performance of LAMOST 1D Pipeline, a new stellar spectral classification template library is constructed.
About 750,000 spectra are chosen from the first data release of LAMOST.
All spectra are divided into different groups by our proposed pseudo color $g^*-r^*$.
Additional groups are formed by different subclasses classified the current pipeline.
In addition, some special types of psectra are manually picked out to add into the groups.
And then these spectra are reduced by our proposed method, which use PCA to reconstruct the original spectra to get spectra of high quality and LoOP to exclude outliers in each group.
The weighted average spectra in each group are treated as the constructed template spectra.
By automatically matching with two known MK class standard spectra library and visual inspection, each spectrum is labeled a MK class.
Also some incorrect spectra are excluded.


However, there are also some problems needing to solve.
We notice that most of our template spectra are those of main sequence stars.
Some additional labeled spectra such as A-type giants and K-type giants are needed to construct the supplemented templates following the method proposed in this paper.
In addition,  some rare  rare types of stars such DC and DZ white dwarfs , magnetic white  dwarfs and carbon white dwarfs, also need to construct  template spectra.
...
and are grateful to  Prof. Yang Haifeng,Dr.Song Yihan, Mrs Re Juanjuan ,Hou Wen for useful suggestions on methods and analysis of the results.
This work is supported by the National Natural Science Foundation of China (Grant No.  61202315, 11078013  and 11203045,11303036).
